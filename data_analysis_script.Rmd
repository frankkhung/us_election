...
abstract: |
  ...
  with Post-Stratification (MRP). This analysis is useful and 
  important because we included all the basic yet crucial information 
  that can easily be obtain from the public. With our model, broader 
  predictions can be made. The model that we are using 
  ...
  data, we can conclude that Joe Biden wins 53% the popular vote with 0.17 
  variance and 52% the electoral vote. In addition to the voting prediction, we
...
# import the librarys needed for the analysis
library(readr)
library(brms)
library(tidyverse)
library(reshape2)
...
# get the data from the input folder
...
# fit the model with brm function with bernoulli distribution
# set the seed for reproducibility and the priors
...
# save the model into voting_fit.rda and place it in the output file, so we don't have the run the above model again later on 
save(voting_fit, file = "outputs/voting_fit.rda")
```


```{r, echo=FALSE, results='hide'}
# load the voting_fit.rda that we saved above 
load(file = "outputs/voting_fit.rda")
# Find the correct classification rate by fitting the original data
Prob <- predict(voting_fit, type = "response")
# In this case, if the probability is larger than 0.5, then it's classified as Donald Trump voteer
...
Prob <- predict(voting_fit, type = "response")
# get the mean vector of the prediction
Prob <- Prob[,1]
# calculate the AUC
...
# Organize the race data for both datas 
...
# assignt the plot to a variable to be used later on in the report
race <- ggplot(data = total_race, aes(x = as.factor(CAT), y = PROB, group = as.factor(TYPE), linetype = as.factor(TYPE))) +
  ...
  scale_linetype_manual(name = "Data Types", values = c("longdash", "solid"), labels = c("Nationscape Data", "ACS Data")) + 
...
# remove large datasets to clean up space for the memory 
rm(ind_level)
# Perform prediction on the post-stratification data 
# the predictions have to be done separately because of the hardware constraint
...
# safe the first few prediction just in case
...
# save the rest of the prediction
...
# load the prediction from both files saved above
load(file = "outputs/post_predict2_9_v2.rda")
load(file = "outputs/post_predict10_16_v2.rda")
# combine all the prediction into one since we separated them at first
...
# combine the prediction with the original post-stratified data
post_predict_level <- cbind(post_level, post_predict_total)

# convert the probabilties to the candidates > 0.5 --> Joe Biden < 0.5 --> Donald Trump
...
```{r, echo=FALSE, results='hide', message = FALSE}
# plot the comparison of the voting situation of both data
ind_race <- ind_level %>%
...
# assign the plot to a variable for future use 
...
```{r, echo=FALSE, results='hide', message=FALSE}
# find the proportion of votes in each state for both candidates
post_state_predict <- post_predict_level %>%
  group_by(state, vote) %>%
  summarise(n = n()) %>%
  mutate(prop = prop.table(n))
# Extract one candidate for the calculation of electoral college votes
biden <- post_state_predict[which(post_state_predict$vote == 'Joe Biden'),]
# electoral college votes based on the states order in post_state_predict
...
# filter out the losing states for Biden
biden_elec <- biden[which(biden$prop >= 0.5),]
# add up the electoral college vote
...
Nationscape Data (Tausanovitch, Chris and Lynn Vavreck. (2020)) and American Community Survey (ACS) Data
(U.S. Census Bureau. (2012)). This allows us to perform logistic regression
...
policies towards certain income levels of people. We will also consider which candidate wins in the Electoral
College.

In the Data section of the report, we will talk about the data thoroughly on  how the data was obtained,
the surveys were asked, and the reasoning behind the variables that we choose. After the Data section, we
...
Despite the convenience of the MRP model, there are still some weaknesses. Some of the weaknessess and limitations also occur in our analysis. Firstly, our model would fail to have good prediction due to insufficient amount of data, or the individual level data (sample data) creates a certain bias towards the population. Secondly, since individual level data and post-stratification level data need to have identitical variables and responses categories, the generalization of data from one to another often loses some detailed information during the process. 
...
The first data that we are going to use is from Democracy Fund + UCLA Nationscape (Tausanovitch, Chris and Lynn Vavreck. (2020)). It is a partnership
...
mid-sized U.S. cities. Nationscape aims to understand people's opinions on the 2020 election. It
...
platform is called Programmatic Sampling, which automates buying and selling the sample (Patrick Comer (2019)).
Using this method, Nationscape could get an enormous amount of data with less cost and human resources.
The platform user accept the selection from the party that creates the survey and answer the survey 
 survey on the platform. There are roughly 12% of the selected people or groups declined to do the survey, 
...
As mentioned above, the sets of data are collected weekly. Here, we are going to pick the results gathered during the week of 
June 25th, 2020. As mentioned in the Introduction section, this research aims to predict the winner of the upcoming 2020 United
States Election. There are several fundamental elements from the respondents that would affect the election outcome, for 
example, the necessary information such as gender, age, race, and individual information such as education level and income 
level. There are two reasons why we decide to use these variables. Firstly, it is general enough to cover the respondents' 
situations, yet not too detail to make the analysis too complicated. Secondly, since we perform multilevel modeling with 
post-stratification, the variables from two datasets have to match.
...
Operations Plan (U.S. Census Bureau. (2012)). ACS aims to provide information to federal, state, and local governments. The ACS
...
# called the assigned plot from above
race
```

- **Voting in 2020 (Nationscape; Figure 9 lower level )**: Next, we will discuss the essential modification on the Nationscape 
...
```{r, echo=FALSE}
# list out the shorter version of the coefficients
fixed_effects <- as.data.frame(fixef(voting_fit))
kable(rbind(tail(fixed_effects), head(fixed_effects)), caption = "Head and Tails of the coefficients of the estimates", digits = 3)
```
...
# create a table for popular votes
...
# plot the vote counts in each state
...
# plot the heatmap for each states
...
# called the plots that were assigned to variables and put them into one graph 
...
- Rohan Alexander and Samantha Caetano (2020) 01-data_cleaning-survey.R
- Rohan Alexander and Samantha Caetano (2020) 01-data_cleaning-survey-post-strat.R
...
- Jona Gabry, Daniel Simpson, Aki Vehtari, Michael Betancourt, Andrew Gelman (2019). “Visualization in Bayesian
...
- Matthew Kay(2020). _tidybayes: Tidy Data and Geoms for Bayesian Models_. doi:
...
# Appendix
```{r, echo = FALSE}
# full table for the coefficients
kable(exp(fixef(voting_fit)), caption = "Parameter Estimate for our model", digits = 3)
```
